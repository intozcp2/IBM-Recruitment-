...
app.get('/product', function (req, res) {
  db.all("SELECT * FROM product where product.obsoleted == '0'", (err, rows) => {
    if (err) {
...
app.get('/membership', function (req, res) {
  var grade = req.query.grade.toUpperCase();
  console.log(grade);
   let sql = `SELECT * FROM membership where membership.grade = '${grade}'`;
   console.log(sql);
   db.all(sql, (err, rows) => {
     if (err) {
       res.status(400).json({ "error": err.message });
...
app.post('/cart', function (req, res) {
  ...
  db.run(`INSERT INTO cart_item (product_id) VALUES (${req.query.productId})`, (err) => {
    ...
    db.all(`SELECT p.id,p.name,sum(p.price) as price, count(p.id) as quantity FROM cart_item ct INNER JOIN product p on ct.product_id = p.id group by p.id`, (err, rows) => {
